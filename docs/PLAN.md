...
- **V5 Schema + Coordinator（本提交）**：`graph.ts` 加 V5 schema（RouterNode/ProjectKnowledgeNode/DocumenterNode + RouteEdge/ObserveEdge + forward/gate 加 lanes/minRisk + GraphV5Schema）+ `validateV5Topology`（Input 唯一连 Router、Router 唯一无 gate/rework、每 lane 单入口、Knowledge/Documenter 仅 observe 不入主路径、route:router→agent、observe:knowledge→documenter、work forward≤1、gate order 连续、decision gate-in/rework-out 一致）+ `validateV5Runnable`（每非 clarify/unsupported lane 经 forward 链确定性到 End；forward 无 lanes 永远活跃、带 lanes 仅匹配 lane 活跃）+ parse schemaVersion 5 + `writeProjectGraph` 拒绝 V5→V4/V3 降级 + stable edge ID（V5 不按 source/target 重写）+ `validateProjectRun`/`snapshotRubrics` 支持 V5。`routing.ts`：RouteDecision（zod）+ CreateRunRequest（IntentMode 兼容 runMode:auto→auto/full→change）+ CapabilityProfile + `routerPrompt`（禁工具/禁读仓库/禁回答）+ `parseRouteDecision`（malformed 抛错）+ `validateRouteDecision`（answer/inspect 不升级写、change 不走 direct、auto 写通道须声明 sideEffects，回落 lane）+ `resolveCapabilityProfile`（Implementer 读写+执行、Verifier 读+测试、critical 外部副作用 human_approval）+ `resolveLanePlan`（lane→route 边→entry→forward 链→End，收集 gates）。`V5Router.ts` `walkV5Graph`：input→Router（fresh，malformed 重试1次→investigate 兜底）→validate→低置信进 Investigator→最多 reroute 一次→仍不确定→clarify(needs_input)→resolveLanePlan→发 `run_plan_created`+`route_decided`→walkLane（forward 链 + V4 式 gate 循环 approve/revise/best-effort，复用 evaluation helpers；exhausted/blocked→run_error 优雅失败，durable pause 留后续）。`AgentRouter.executeGraph` V5 分流（intentMode 从 runMode 派生）。`projects.ts`/start 对 V5 也快照 rubrics。前端 `graphRunStore` 加 `run_plan_created` 事件 + bubble。测试：v5-graph(9)+routing(11)+v5-router(5)。

**累计测试 136 过**（V5 新增 25 例）；api+web typecheck 绿；web build 绿；启动烟测 /api/providers 200。
...
- **V5 schema + Coordinator 完成（commit #3）**。下一步 commit #4 V5 角色/模板（默认路线 7 lane + 高风险 Security Reviewer + 发布部署迁移只产计划暂停 + 新项目默认 V5 模板 + opencode:project-router 节点配置 + 缺 Provider 明确报告）→ #5 Project Knowledge → #6 Scribe → #7 画布/Issues UI → #8 能力策略/迁移文档。
